...
( function() {
	...
	//	scope.animations = ( new Animation() ).parse( nodes, scope.hierarchy );
		...
		scope.textures = undefined
		//scope.conns = (new Connections()).traverseConnection(nodes)
        //nodes.traverseConnection();
        scope.prop = (new buildingProps()).parse(nodes)
        console.log(scope.prop)
        
		console.timeEnd( 'FBXLoader: ObjectParser' );

		//scope.materials = this.parseMaterials(nodes)
		scope.materials = ( new Materials() ).parse(nodes);
		...
		for ( var i = 0; i < geometries.length; ++ i ) {
			...
			//yhe muilt material
			if (geometries[ i ].material  instanceof THREE.MultiMaterial){
				console.log(1)
			}
			// 	children = geometries[ i ]
	...
	//	geo.addBones( this.hierarchy.hierarchy );
		...
		var layerMaterial = geo.node.subNodes.LayerElementMaterial

		var pipe = geo.id ==1811867984
		var wall = geo.id ==1744996128// ==1744997808
		var table = geo.id ==2035697072
		var roof = geo.id==1744995568
		var ground = geo.id == 1744995008
		var glass = geo.id ==1791741360
		var windFlag = geo.id ===1744998512 || geo.id ===1745000752 ||geo.id ===1754712624
		var vase =  geo.id === 2035692032

		var mullions = [1745075584,1751623440,1751621760,1751622320,1814746112 ]
		mullions = [ 1784797056 , 1784797616 ,1784798176,1784798736,1784799296,1784799856 , 1784801536
		,1744984928 , 1811588272 , 1786438288, 1786438848 , 1786439408 , 1786439968 , 1784798176
		 ]
		var fushou = [ 1786448368 , 1786448928 , 1786449488 , 1786450048 , 1791713920]
		var fFlag = (fushou.indexOf(geo.id)!== -1 )
		//mullions = [ 1744984928 ]
		var mFlag = (mullions.indexOf(geo.id)!== -1 )
		// if (fFlag !==true){
...
//yhe changing the material and texture
		...
		if (this.materials.materials == undefined){
			material =undefined
		}
		//yhe todo material layer
		else{
			var matList = this.materials.getById(nodes.searchConnectionParent(geo.id))
			if (matList ===undefined)
				material = undefined
			else{
				material = matList[matList.length - 1].material
			}
		}

		if (material === undefined){
			material = new THREE.MeshPhongMaterial()
			material.color = new THREE.Color(0x8e8e8e)
			material.specular = new THREE.Color(0x8e8e8e)
			material.emissive = new THREE.Color(0x8e8e8e)
		}
		...
		// if (geo.isMultiMaterials ===true){
		// 	//var geoGlobal = new THREE.Geometry().fromBufferGeometry( geometry );

		// 	var geoList = []
		// 	for (var i = 0 ; i<geo.materialCount ; i++){
		// 		var g1 = new THREE.Geometry().fromBufferGeometry( geometry );
		// 		var faces = g1.faces
		// 		g1.faces=[]
		// 		var matIndex = geo.matIndex[i]
		// 		for (var j = 0 ; j<matIndex.length ; j++ ){
		// 			g1.faces.push(faces[j])
		// 		}
		// 		geoList.push(g1)
		// 	}

		// 	//yhe to do multi material 
		// 	// with material lists and face index


		// 	//console.log(geoList)
		// 	// var a = new THREE.Group();
		// 	// for (var i =0 ; i<geoList.length ; i++){
		// 	// 	//var m1 = new THREE.Mesh( geoList[i], new THREE.MeshBasicMaterial());
		// 	// 	a.add(new THREE.Mesh( geoList[i], new THREE.MeshBasicMaterial()))
		// 	// }
		// 	return geoList
		...
		var prop = this.prop.getById(nodes.searchConnectionParent(geo.id))
		...
		// 	for (var i = 0 ; i<geo.layerMaterial.length ; i++){
		// 		geometry.faces[i].materialIndex = geo.layerMaterial[i]
		// 	}
		// }
		// else {
		...
		if ( geo.bones === undefined || geo.skins === undefined || this.animations === undefined || this.animations.length === 0 ) {
			...
			if (geo.isMultiMaterials ===true && matList !==undefined && (matList.length>1 )){
				//if the geometry is multi material
				...
				for (var i =0 ; i<matList.length ;i++){
					m.push(matList[i].material)
				}


				diff = geo.materialCount - matList.length
				for (var i = 0 ; i<diff ; i++){
					var a = new THREE.MeshPhongMaterial(( { color: Math.random() * 0xffffff } ))
					//a.opacity = 0.0
					m.push(a)
				}
				
				// for (var i = 0 ; i<geo.materialCount ; i++){
				// 	var a = new THREE.MeshPhongMaterial(( { color: "rgb(255, 0, 0)"} ))
				// 	a.specular= new THREE.Color(  "rgb(0, 255, 0)")
				// 	//a.opacity = 1.0- Math.random()

				// 	a.transparent = true
				// 	m.push(a)
				// }
				//geometry = new THREE.BufferGeometry().fromGeometry( geometry );
				mesh = new THREE.Mesh( geometry, new THREE.MultiMaterial(m)   );
				//mesh = new THREE.Mesh( geometry, new THREE.MultiMaterial(matList)   );
			}
			else {
			...
			// var m = new THREE.MultiMaterial(matList)  
			// mesh = new THREE.Mesh( geometry, m  );
			//mesh = new THREE.Mesh( geometry, material );
			mesh.userData = prop
			//mesh.userDate.name = prop.attrName
		...
		mesh = this.lclTransform(mesh , prop.property)
	...
	THREE.FBXLoader.prototype.lclTransform = function(mesh,prop){
		var flagTranslate = "Lcl_Translation" in prop
		var flagRotate = "Lcl_Rotation" in prop
		var flagScale = "Lcl_Scaling" in prop
		var flagPreRota = "PreRotation" in prop
		// flagTranslate=false
		// flagScale=false
		// flagRotate=false
		// flagPreRota = false
		...
		if (flagPreRota ===true){
			var preRota = prop.PreRotation.value
			for (var p in preRota){
				if (preRota.hasOwnProperty(p))
					preRota[p]*=(Math.PI/180)
			}
			//preRota.x*=(Math.PI/180)
			
			var pX = preRota.x
			var pY = preRota.y
			var pZ = preRota.z

			//rotateAroundWorldAxis(mesh , new THREE.Vector3(1,0,0) ,  pX )
			// mesh.rotateOnAxis(new THREE.Vector3(1,0,0) ,pX )
		...
		if (flagTranslate ===true){

			var translate = prop.Lcl_Translation.value.split(",")
			var tX = Number(translate[0])
			var tY = Number(translate[1])
			var tZ = Number(translate[2])

			// if (flagScale ===true){
			// 	var scale = prop.Lcl_Scaling.value.split(",")
			// 	var sX = Number(scale[0])
			// 	var sY = Number(scale[1])
			// 	var sZ = Number(scale[2])
			// 	tX=tX/sX
			// 	tY=tY/sY
			// 	tZ=tZ/sZ				
			// }
		...
		if (flagRotate ===true){
			var rotate = prop.Lcl_Rotation.value.split(",")
			var rX = Number(rotate[0])*(Math.PI/180)
			var rY = Number(rotate[1])*(Math.PI/180)
			var rZ = Number(rotate[2])*(Math.PI/180)
			// mesh.rotation.x = Math.PI/4
			// mesh.rotation.y = 4*Math.PI/4

			mesh.rotation.x =  mesh.rotation.x+ rX 
			mesh.rotation.y = mesh.rotation.y+ rY //+ Math.PI/2
			mesh.rotation.z = mesh.rotation.z+ rZ //+ Math.PI/2
		}
    ...
    Materials.prototype.parse = function (nodes){
    	var subNodes = nodes.Objects.subNodes
    	if ( ! ( 'Material' in subNodes ) ) {
			this.materials = undefined
			this.modelMap = undefined
			return this
		}
		var matCount = 0;
		for ( var mat in subNodes.Material ) {
		...
		if ( matCount > 0 ) {
			...
			for ( mat in nodes.Objects.subNodes.Material ) {

				var material = (new Material()).parse(subNodes.Material[mat],nodes);
				this.add(material)
				//res.push( this.parseMaterial( nodes.Objects.subNodes.Material[ mat ],nodes ) );
				//parseGeometry( node.subNodes.Material[ mat ],node ) 

			}

		} else {
			var material = (new Material()).parse(subNodes.Material,nodes);
			this.add(material)
			//res.push( this.parseMaterial( nodes.Objects.subNodes.Material,nodes ) );

		}
		return this
    };

    Materials.prototype.add = function(material){
    	this.materials.push(material)


    	for ( var i = 0; i <material.modelId.length; ++i){
    		if (this.modelMap[material.modelId[i]] === undefined){
    			this.modelMap[material.modelId[i]] = []
    		}
    		this.modelMap[material.modelId[i]].push( material)
    	}
    ...
    Material.prototype.parse = function(node,nodes){
        ...
        this.id = node.id
        this.modelId = nodes.searchConnectionParent(this.id) 
        
        if (this.modelId ===1962320736)
        	console.log("yes")

		var dColor=props.DiffuseColor.value
		var r=Math.round(dColor.x*255).toString()
		var g=Math.round(dColor.y*255).toString()
		var b=Math.round(dColor.z*255).toString()


		this.material = new THREE.MeshPhongMaterial( { color: "rgb(" + r + "," + g + "," + b + ")" } );
		//this.material = new THREE.MeshLambertMaterial( { color: "rgb(" + r + "," + g + "," + b + ")" } );
		var sColor = props.SpecularColor.value
		var sr=Math.round(sColor.x*255).toString()
		var sg=Math.round(sColor.y*255).toString()
		var sb=Math.round(sColor.z*255).toString()

		
		
		this.material.specular= new THREE.Color("rgb(" + sr + "," + sg + "," + sb + ")" )

		var trans = props.TransparencyFactor.value
		this.material.opacity = 1.0-trans

		this.material.transparent = true
		//this.material = new THREE.MeshBasicMaterial( { color: "rgb(" + r + "," + g + "," + b + ")" } );
    ...
    buildingProps.prototype.parse = function(nodes){
    
		var model = nodes.Objects.subNodes.Model
        for (m in model){
           // console.log(m)
            var prop = (new buildingProp()).parse(m , model);
            this.add(prop)
        }
        return this
    }
	
    buildingProps.prototype.add = function (prop){
       //if (prop === undefined)
        this.props.push(prop)
        this.modelMap[prop.id] = prop

    }

    buildingProps.prototype.getById = function (id){
    	return this.modelMap[id]
    }
    ...
    buildingProp.prototype.parse = function(m, model){
        this.id = m
        this.name = model[m].attrName
        this.property=model[m].properties
        return this
    }
    ...
    //yhe traveser connection
    FBXNodes.prototype.traverseConnection = function(){
		var conns = this.Connections.properties.connections;
        var obj = this.Objects.subNodes;
        for (o in obj){
            //console.log(o)
            if (o.toUpperCase() === 'model'.toUpperCase() ||
            o.toUpperCase() === 'geometry'.toUpperCase()){
                for (id in obj[o]){
	...
	Geometry.prototype.parse = function ( geoNode ) {
		...
		var menban = geoNode.id ===1744997808
		if (menban ===true)
			console.log(menban)

		//layer material
		this.layerMaterial=geoNode.subNodes.LayerElementMaterial
		if (this.layerMaterial.properties.MappingInformationType ==="AllSame"){
			this.isMultiMaterials =false
		}
		else
			this.isMultiMaterials = true
		this.layerMaterial = this.getlayerMaterial()
	...
	//yhe get layerelement material
	Geometry.prototype.getlayerMaterial = function () {
		if ( this.node.__cache_material ) {
			return this.node.__cache_material ;
		}
		if ( this.node.subNodes.LayerElementMaterial === undefined ) {

			console.warn( 'this.node: ' + this.node.attrName + "(" + this.node.id + ") does not have LayerElementMaterial" );
			this.node.__cache_material = undefined;
			return null;

		}

		var rawMaterial = this.node.subNodes.LayerElementMaterial.subNodes.Materials.properties.a;
		var material = rawMaterial.split(',').map(function(element){
			return Number(element)
		})
		this.node.__cache_material =material

		var count=1
		var a = material.concat().sort()
		var matIndex={}
		matIndex[a[0]]=[0]
